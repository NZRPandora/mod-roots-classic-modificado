...
public class ComponentSunflower
    ...
    public void doEffect(Level level, Entity casterEntity, EnumCastType type, double x, double y, double z, double potency, double duration, double size) {
        if (type == EnumCastType.SPELL && casterEntity instanceof LivingEntity) {
            ...
            for (LivingEntity target2 : targets) {
                ...
                if (target2.isInvertedHealAndHarm()) {
                    target2.hurt(ComponentSunflower.spellAttack(caster), (float)(damageDealt += (int)(5.0 + 4.0 * potency)));
                    target2.igniteForSeconds((float)damageDealt);
                    target2.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 15, 2 + (int)potency));
                    target2.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 15, 2 + (int)potency));
                } else {
                    damageDealt += (int)(3.0 + 2.0 * potency);
                }
                target2.hurt(caster.damageSources().inFire(), (float)damageDealt);
